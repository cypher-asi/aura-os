...
async function main() {
  try {
    ...
    if (!home.ok || !homeHtml.includes("Aura Patch Complete") || !homeHtml.includes("Patched by Aura")) {
      throw new Error(`Homepage verification failed. Status=${home.status}\n${homeHtml}`);
    }

    const health = await fetch(`http://127.0.0.1:${port}/health`);
    const healthJson = await health.json();
    if (!health.ok || healthJson.status !== "ok" || healthJson.source !== "patched") {
      throw new Error(`Health verification failed: ${JSON.stringify(healthJson)}`);
    }

    console.log("Existing server patch smoke test passed.");
  } finally {
    child.kill("SIGTERM");
  }
}
