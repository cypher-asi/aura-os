...
export async function installChatCoreMockApp(
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (pathname === "/api/auth/session" || pathname === "/api/auth/validate") return json(route, session);
    ...
    if (pathname === `/api/projects/${scenario.project.projectId}/agents/${scenario.agent.agentInstanceId}/events`) {
