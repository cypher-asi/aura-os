...
vi.mock("./ProjectWorkScreen.module.css", () => ({
  default: new Proxy({}, { get: (_target, prop) => String(prop) }),
}));

import { MobileProjectWorkScreen } from "./ProjectWorkScreen";
...
describe("MobileProjectWorkScreen", () => {
  it("keeps the mobile work flow focused on execution and specs", () => {
    render(<MobileProjectWorkScreen />);
  ...
  it("uses the route project id when project context has not resolved yet", () => {
    mockUseProjectContext.mockReturnValue(null);

    render(
      <Routes>
        <Route path="/projects/:projectId/work" element={<MobileProjectWorkScreen />} />
      </Routes>,
      { routerProps: { initialEntries: ["/projects/proj-from-route/work"] } },
    );

    expect(mockUseMobileTasks).toHaveBeenCalledWith("proj-from-route");
    expect(screen.getByTestId("mobile-project-work")).toBeInTheDocument();
  });
});
