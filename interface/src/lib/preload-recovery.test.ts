...
describe("installPreloadRecovery", () => {
  ...
  it("does not reload again after recovery has already been attempted in the same tab session", () => {
    let handler: ((event: Event) => void) | null = null;
    const runtimeWindow = {
      addEventListener: vi.fn((event: string, listener: (event: Event) => void) => {
        if (event === "vite:preloadError") {
    ...
    expect(runtimeWindow.location.reload).toHaveBeenCalledTimes(1);
    expect(errorSpy).toHaveBeenCalled();
    errorSpy.mockRestore();
  });

  it("clears the recovery flag after a successful boot so future mismatches can recover again", () => {
    const sessionStorage = createSessionStorage({ "aura-preload-recovery": "1" });
    const runtimeWindow = {
    ...
    expect(runtimeWindow.sessionStorage.getItem("aura-preload-recovery")).toBeNull();
  });
});
