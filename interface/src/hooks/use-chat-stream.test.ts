...
vi.mock("../api/streams", () => ({
  ...
  // `useChatStream` reattach imports + the transitive `shared/api/agents`
  // re-export pull these named exports off the mocked module; they must
  // be present or vitest throws "No <name> export is defined on the mock"
  // at module-eval time (before any test runs).
