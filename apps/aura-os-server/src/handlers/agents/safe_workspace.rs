...
async fn authorize_session(
    ...
    let matches_project = session.project_id.as_deref() == Some(project_id.to_string().as_str());
    let matches_agent =
        session.project_agent_id.as_deref() == Some(agent_instance_id.to_string().as_str());
    if !matches_project || !matches_agent {
