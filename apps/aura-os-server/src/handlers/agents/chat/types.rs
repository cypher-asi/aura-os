...
use super::constants::{
    HEADER_CHAT_COMMAND_ID, HEADER_CHAT_PERSISTED, HEADER_CHAT_PROJECT_ID, HEADER_CHAT_SESSION_ID,
};
...
mod tests {
    ...
    fn accepted_command_receipt_is_correlated_to_persisted_session() {
        ...
        let headers = sse_response_headers(Some(&snapshot), Some("mobile-123"));
    ...
    fn unpersisted_response_never_claims_a_command_receipt() {
        let headers = sse_response_headers(None, Some("mobile-123"));
