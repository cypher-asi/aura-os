...
impl ProcessScheduler {
    pub fn new(executor: Arc<ProcessExecutor>, storage_client: Option<Arc<StorageClient>>) -> Self {
        Self {
            executor,
            storage_client,
        }
    ...
    async fn tick(&self) -> Result<(), String> {
        let client = self
            .storage_client
            .as_ref()
            .ok_or_else(|| "aura-storage is required for process scheduling".to_string())?;
        if !client.has_internal_token() {
            return Err(
                "AURA_STORAGE_INTERNAL_TOKEN is required for scheduled process execution"
                    .to_string(),
            );
        }
